...
public class UserClientService {
    ...
    public boolean checkUser(String userId, String pwd) {
        ...
        //连接到服务端，发送u对象
        try {
            socket = new Socket("10.160.133.3", 9999);
            ...
            try {
                ...
                if (ms.getMesType().equals(MessageType.MESSAGE_LOGIN_SUCCEED)) {//登录成功
                    ...
                    //如果登录失败，我们就不能启动和服务端通信的线程，关闭socket
                    socket.close();
                }
    ...
    public void logout() {
        ...
        message.setSender(u.getUserId());//一定要知名那个客户端
        //发送message
        try {
            ObjectOutputStream oos =
                    new ObjectOutputStream
    ...
    public boolean createUser(String userid,String pwd){
        boolean loop=false;
        User createUser=new User(userid,pwd);
        createUser.setCreate(MessageType.MESSAGE_CREATE);
        //连接到服务端，发送u对象
        try {
            ...
            try {
                Message ms = (Message) ois.readObject();
                if(ms.getMesType().equals(MessageType.MESSAGE_CREATE_SUCCEED)){
                    loop=true;
                }
            } catch (ClassNotFoundException e) {
                e.printStackTrace();
